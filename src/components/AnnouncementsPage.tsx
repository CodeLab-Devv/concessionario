...
export const AnnouncementsPage: React.FC = () => {
  ...
  const deleteAnnouncement = async (announcement: Announcement) => {
    if (!canManage) return;
    if (!window.confirm(`Eliminare l'annuncio "${announcement.title}"? Questa operazione non può essere annullata.`)) return;
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-5">
      ...
      {selected && <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/40 p-0 sm:items-center sm:p-4" onClick={() => setSelected(null)}><div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-2xl" onClick={event => event.stopPropagation()}>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t pt-4"><div className="flex items-center gap-2 text-sm text-gray-500"><Check className="h-4 w-4 text-emerald-500" />{selected.read ? 'Visualizzato' : 'Presa visione registrata all’apertura'}</div><div className="flex items-center gap-2">{canManage && <><button onClick={() => startEdit(selected)} className="inline-flex items-center gap-2 rounded-lg border border-amber-200 px-3 py-2 text-xs font-medium text-amber-700 hover:bg-amber-50"><Pencil className="h-4 w-4" />Modifica</button><button onClick={() => void deleteAnnouncement(selected)} className="inline-flex items-center gap-2 rounded-lg border border-red-200 px-3 py-2 text-xs font-medium text-red-600 hover:bg-red-50"><Trash2 className="h-4 w-4" />Elimina</button><button onClick={() => void loadViewers(selected)} className="inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-xs font-medium text-gray-700 hover:bg-gray-50"><Eye className="h-4 w-4" />Visualizzazioni</button></>}</div></div>
