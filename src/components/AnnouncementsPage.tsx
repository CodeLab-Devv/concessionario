...
import { Bell, Check, ChevronRight, Clock3, Eye, Megaphone, Plus, Send, Users, X } from 'lucide-react';
...
export const AnnouncementsPage: React.FC = () => {
  ...
  const loadAnnouncements = useCallback(async () => {
    ...
    try {
      ...
      setAnnouncements(rows.map(row => ({ ...row, author: authorMap.get(row.author_id), read: readMap.has(row.id) })));
    } catch (error) {
  ...
  useRealtimeSubscription({
    table: 'announcements',
    onInsert: () => { void loadAnnouncements(); },
    onUpdate: () => { void loadAnnouncements(); },
    onDelete: () => { void loadAnnouncements(); },
    enabled: Boolean(user?.id)
  });

  const openAnnouncement = async (announcement: Announcement) => {
    setSelected(announcement);
    if (!user?.id || announcement.read) return;
  ...
  const publish = async () => {
    if (!user?.id || !canPublish || !title.trim() || !content.trim()) return;
    setSaving(true);
    try {
      const { error } = await supabase.from('announcements').insert({ author_id: user.id, title: title.trim(), content: content.trim() });
      if (error) throw error;
      setTitle(''); setContent(''); setShowComposer(false); await loadAnnouncements();
    } catch (error) {
      alert(`Errore pubblicazione annuncio: ${(error as Error).message}`);
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2"><Megaphone className="h-6 w-6 text-amber-600" /><h1 className="text-2xl font-bold text-gray-900">Annunci</h1>{unreadCount > 0 && <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-700">{unreadCount} nuovi</span>}</div>
          <p className="mt-1 text-sm text-gray-500">Comunicazioni del concessionario, con conferma di presa visione.</p>
        </div>
        {canPublish && <button onClick={() => setShowComposer(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-yellow-500 to-amber-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:from-yellow-600 hover:to-amber-700"><Plus className="h-4 w-4" />Nuovo annuncio</button>}
      </div>

      {showComposer && <div className="rounded-2xl border border-amber-100 bg-white p-4 shadow-sm sm:p-5">
        <div className="mb-4 flex items-center justify-between"><h2 className="font-semibold text-gray-900">Pubblica annuncio</h2><button onClick={() => setShowComposer(false)} className="rounded-lg p-2 text-gray-400 hover:bg-gray-100"><X className="h-4 w-4" /></button></div>
        <div className="space-y-3">
          <input value={title} onChange={event => setTitle(event.target.value)} maxLength={120} placeholder="Titolo dell'annuncio" className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm outline-none transition focus:border-amber-400 focus:ring-2 focus:ring-amber-100" />
          <textarea value={content} onChange={event => setContent(event.target.value)} maxLength={10000} rows={5} placeholder="Scrivi il messaggio per il team..." className="w-full resize-y rounded-xl border border-gray-200 px-4 py-3 text-sm outline-none transition focus:border-amber-400 focus:ring-2 focus:ring-amber-100" />
          <div className="flex justify-end"><button disabled={saving || !title.trim() || !content.trim()} onClick={() => void publish()} className="inline-flex items-center gap-2 rounded-xl bg-gray-900 px-4 py-2.5 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"><Send className="h-4 w-4" />{saving ? 'Pubblicazione...' : 'Pubblica'}</button></div>
        </div>
      </div>}

      {loading ? <div className="rounded-2xl border bg-white p-8 text-center text-sm text-gray-500">Caricamento annunci...</div> : announcements.length === 0 ? <div className="rounded-2xl border bg-white p-10 text-center"><Bell className="mx-auto h-9 w-9 text-gray-300" /><p className="mt-3 font-medium text-gray-700">Nessun annuncio</p><p className="mt-1 text-sm text-gray-400">Le comunicazioni del team appariranno qui.</p></div> : <div className="grid gap-3 md:grid-cols-2">{announcements.map(announcement => <button key={announcement.id} onClick={() => void openAnnouncement(announcement)} className={`group text-left rounded-2xl border bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${announcement.read ? 'border-gray-100' : 'border-amber-200 bg-amber-50/30'}`}>
        <div className="flex gap-3">
          <div className="h-11 w-11 shrink-0 overflow-hidden rounded-full bg-gray-100 ring-1 ring-gray-200">{announcement.author?.avatar_url ? <img src={announcement.author.avatar_url} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center text-sm font-semibold text-gray-500">{announcement.author?.name?.slice(0,1).toUpperCase() || '?'}</div>}</div>
          <div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><h3 className="truncate font-semibold text-gray-900">{announcement.title}</h3>{!announcement.read && <span className="shrink-0 rounded-full bg-amber-500 px-2 py-0.5 text-[10px] font-bold uppercase text-white">Nuovo</span>}</div><p className="mt-0.5 text-xs text-gray-500">{announcement.author?.name || 'Utente'} · {formatDate(announcement.created_at)}</p><p className="mt-2 line-clamp-2 text-sm leading-5 text-gray-600">{announcement.content}</p></div><ChevronRight className="mt-3 h-4 w-4 shrink-0 text-gray-300 transition group-hover:translate-x-0.5" /></div>
      </button>)}</div>}

      {selected && <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/40 p-0 sm:items-center sm:p-4" onClick={() => setSelected(null)}><div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-2xl" onClick={event => event.stopPropagation()}>
