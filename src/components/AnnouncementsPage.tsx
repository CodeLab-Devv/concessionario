...
export const AnnouncementsPage: React.FC = () => {
  ...
  const loadAnnouncements = useCallback(async () => {
    ...
    try {
      ...
      setAnnouncements(rows.map(row => ({
        ...
        // Il creatore vede sempre il proprio annuncio come già visualizzato.
        read: row.author_id === user.id || readMap.has(row.id)
      })));
    } catch (error) {
  ...
  const openAnnouncement = async (announcement: Announcement) => {
    ...
    // L'autore non deve creare una presa visione per il proprio annuncio.
    ...
    <div className="mx-auto w-full max-w-6xl space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Comunicazioni del concessionario, con conferma di presa visione. Clicca sull'annuncio per visualizzare e leggere il contenuto.</p>
        </div>
      ...
      {selected && <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/40 p-0 sm:items-center sm:p-4" onClick={() => setSelected(null)}><div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-2xl" onClick={event => event.stopPropagation()}>
        ...
        <div className="mt-5 flex items-center justify-between gap-3 border-t pt-4"><div className="flex items-center gap-2 text-sm text-gray-500"><Check className="h-4 w-4 text-emerald-500" />{selected.read ? 'Visualizzato' : 'Presa visione registrata all’apertura'}</div>{canPublish && <button onClick={() => void loadViewers(selected)} className="inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-xs font-medium text-gray-700 hover:bg-gray-50"><Eye className="h-4 w-4" />Visualizzazioni</button>}</div>
