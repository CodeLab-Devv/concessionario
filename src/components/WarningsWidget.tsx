...
export const WarningsWidget: React.FC<{ mode: 'dashboard' | 'manage' }> = ({ mode }) => {
  ...
  const warningCountByEmployee = useMemo(() => warnings.reduce<Record<string, number>>((counts, warning) => {
    counts[warning.employee_id] = (counts[warning.employee_id] || 0) + 1;
    return counts;
  }, {}), [warnings]);

  const load = useCallback(async () => {
    if (!user?.id) return;
    const { data, error } = await supabase.from('disciplinary_warnings').select('id,employee_id,issued_by,reason,severity,created_at').eq('employee_id', user.id).order('created_at', { ascending: false });
  ...
  const save = async () => {
    ...
    if (form.severity === 'last_chance' && !canAddLastChance) return showWarning('Last Chance non disponibile', 'La Last Chance è disponibile solo dopo 3 richiami e può essere assegnata una sola volta.');
    setSaving(true);
    try {
  ...
  if (mode === 'dashboard') {
    ...
    return <div className="mb-6 rounded-2xl border border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50 p-5 shadow-lg">
      <div className="flex items-center gap-4"><div className="rounded-xl bg-amber-100 p-3 text-amber-600"><AlertTriangle className="h-6 w-6" /></div><div className="min-w-0"><p className="text-sm font-medium text-amber-700">Provvedimenti disciplinari</p><p className="text-2xl font-bold text-gray-900">{hasLastChanceDashboard ? 'Last Chance' : `${regularCount} ${regularCount === 1 ? 'richiamo' : 'richiami'}`}</p><p className="text-sm text-gray-600">{hasLastChanceDashboard ? 'Hai ricevuto una Last Chance.' : `Hai ricevuto ${regularCount === 1 ? 'un richiamo' : `${regularCount} richiami`}.`}</p></div></div>
      <div className="mt-4 space-y-2">{warnings.slice(0, 3).map(warning => <div key={warning.id} className="rounded-lg bg-white/80 px-3 py-2 text-sm text-gray-700"><span className="font-semibold">{warning.severity === 'last_chance' ? 'Last Chance' : warning.severity === 'formale' ? 'Richiamo formale' : 'Richiamo'}:</span> {warning.reason}</div>)}</div>
    </div>;
  }
  ...
  return <div className="rounded-xl bg-white shadow-lg overflow-hidden">
    <div className="flex items-center justify-between border-b border-gray-200 px-4 py-4 sm:px-6"><div><h2 className="text-lg font-semibold text-gray-900">Richiami disciplinari</h2><p className="text-sm text-gray-600">Massimo 3 richiami, poi Last Chance.</p></div><button onClick={() => setShowForm(true)} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-gradient-to-r from-yellow-500 to-amber-600 px-4 py-2.5 font-semibold text-white shadow-md hover:from-yellow-600 hover:to-amber-700"><Plus className="h-4 w-4" /> Nuovo richiamo</button></div>
    <div className="divide-y divide-gray-100">{warnings.length === 0 ? <div className="p-8 text-center text-sm text-gray-500">Nessun richiamo registrato.</div> : warnings.map(warning => <div key={warning.id} className="flex items-start justify-between gap-4 p-5"><div><div className="flex flex-wrap items-center gap-2"><span className="font-semibold text-gray-900">{employeeMap.get(warning.employee_id)?.name || 'Dipendente'}</span><span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-700">{warning.severity === 'last_chance' ? 'Last Chance' : warning.severity === 'formale' ? 'Formale' : 'Richiamo'}</span></div><p className="mt-2 text-sm text-gray-700">{warning.reason}</p><p className="mt-2 text-xs text-gray-500">{new Intl.DateTimeFormat('it-IT', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(warning.created_at))}</p></div><button onClick={() => void remove(warning.id)} className="rounded-lg p-2 text-gray-400 hover:bg-red-50 hover:text-red-600"><Trash2 className="h-4 w-4" /></button></div>)}</div>
    {showForm && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4" onMouseDown={event => { if (event.target === event.currentTarget) setShowForm(false); }}><div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b p-5"><h2 className="text-lg font-bold text-gray-900">Nuovo provvedimento</h2><button onClick={() => setShowForm(false)} className="rounded-lg p-2 text-gray-400 hover:bg-gray-100"><X className="h-5 w-5" /></button></div><div className="space-y-4 p-5"><label className="block"><span className="mb-1.5 block text-sm font-medium text-gray-700">Dipendente</span><select value={form.employeeId} onChange={event => setForm(current => ({ ...current, employeeId: event.target.value }))} className="w-full rounded-lg border border-gray-300 px-3 py-2.5"><option value="">Seleziona dipendente</option>{employees.filter(employee => employee.id !== user?.id).map(employee => <option key={employee.id} value={employee.id}>{employee.name} — {roleLabels[employee.role] || employee.role}</option>)}</select></label><label className="block"><span className="mb-1.5 block text-sm font-medium text-gray-700">Tipo</span><select value={form.severity} onChange={event => setForm(current => ({ ...current, severity: event.target.value as Warning['severity'] }))} disabled={!form.employeeId || (!canAddWarning && !canAddLastChance)} className="w-full rounded-lg border border-gray-300 px-3 py-2.5"><option value="richiamo" disabled={!canAddWarning}>Richiamo ({Math.min(selectedEmployeeWarningCount + 1, 3)}/3)</option><option value="formale" disabled={!canAddWarning}>Richiamo formale ({Math.min(selectedEmployeeWarningCount + 1, 3)}/3)</option><option value="last_chance" disabled={!canAddLastChance}>Last Chance</option></select></label>{form.employeeId && selectedEmployeeWarningCount >= 3 && !hasLastChance && <div className="rounded-lg border border-orange-200 bg-orange-50 px-3 py-2 text-sm text-orange-800">Questo dipendente ha già 3 richiami. È disponibile solo la <strong>Last Chance</strong>.</div>}{form.employeeId && hasLastChance && <div className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-800">La <strong>Last Chance</strong> è già stata assegnata. Non sono disponibili ulteriori provvedimenti.</div>}<label className="block"><span className="mb-1.5 block text-sm font-medium text-gray-700">Motivazione</span><textarea value={form.reason} onChange={event => setForm(current => ({ ...current, reason: event.target.value }))} rows={5} maxLength={2000} placeholder="Descrivi il motivo del provvedimento..." className="w-full resize-y rounded-lg border border-gray-300 px-3 py-2.5" /></label><div className="flex justify-end gap-3 border-t pt-4"><button onClick={() => setShowForm(false)} className="rounded-lg px-4 py-2.5 text-gray-700 hover:bg-gray-100">Annulla</button><button onClick={() => void save()} disabled={saving || !form.employeeId || hasLastChance} className="rounded-lg bg-amber-600 px-4 py-2.5 font-semibold text-white disabled:opacity-50">{saving ? 'Salvataggio...' : form.severity === 'last_chance' ? 'Assegna Last Chance' : 'Assegna richiamo'}</button></div></div></div></div>}
  </div>;
};
