...
export const AvailabilityEditor: React.FC<Props> = ({ value, onChange, compact = false }) => {
  ...
  return (
    <section className="w-full min-w-0 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      <div className="flex min-w-0 items-center gap-3 border-b border-gray-100 px-3 py-2.5 sm:px-4 sm:py-3">
        ...
        <div className="min-w-0">
          <h4 className="text-sm font-semibold leading-tight text-gray-900">Disponibilità</h4>
          <p className="mt-0.5 truncate text-xs text-gray-500">Giorni e orari di lavoro</p>
        </div>
      </div>
      ...
      <div className="divide-y divide-gray-100">
        {AVAILABILITY_DAYS.map(([key, label]) => {
          ...
          return (
            <div
              key={key}
              className="grid min-w-0 grid-cols-1 items-center gap-2 px-3 py-2 sm:grid-cols-[minmax(110px,0.8fr)_minmax(0,1.2fr)] sm:gap-4 sm:px-4"
            >
              <label className="flex min-w-0 cursor-pointer items-center gap-2">
                <span
                  aria-hidden="true"
                  className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-md border transition-colors ${
                    day.enabled ? 'border-blue-600 bg-blue-600 text-white' : 'border-gray-300 bg-white'
                  }`}
                >
                ...
                <input
                  ...
                  className="sr-only"
                />
                <span className={`truncate text-sm font-medium ${day.enabled ? 'text-gray-900' : 'text-gray-500'}`}>
                  {label}
                </span>
              </label>

              {day.enabled ? (
                <div className="flex min-w-0 w-full items-center gap-1.5 sm:justify-end sm:gap-2">
                  ...
                  <input
                    ...
                    className="h-9 min-w-0 flex-1 rounded-lg border border-gray-200 bg-gray-50 px-2 text-sm text-gray-800 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 sm:w-[92px] sm:flex-none"
                  />
                  <span className="shrink-0 px-0.5 text-xs font-medium text-gray-400">—</span>
                  <input
                ...
                <span className="truncate text-xs text-gray-400 sm:text-right">Non disponibile</span>
              )}
            </div>
          );
        })}
