          ...
          <div className={`flex items-center ${isOpen ? 'justify-between' : 'justify-center'}`}>
            ...
            {isOpen && (
              <button
                ...
                className="hidden h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 shadow-sm transition hover:border-amber-200 hover:bg-amber-50 hover:text-amber-600 active:scale-95 lg:flex"
              >
                <X className="h-5 w-5" />
              </button>
            )}
          ...
          {isOpen && user && (
            ...
            >
              <div className="relative shrink-0">
                ...
                <span className={`absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-white transition-colors ${isOnService ? 'bg-emerald-500' : 'bg-slate-300'}`} />
              </div>
        ...
        <nav className="min-h-0 flex-1 overflow-y-auto px-3 py-4 sm:px-4" aria-label="Navigazione principale">
          {isOpen && (
            <div className="mb-3 px-2">
              <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">Menu principale</span>
            </div>
          )}
                ...
                >
                  ...
                  {isOpen && (
                    <span className="min-w-0 flex-1 py-0.5">
                      <span className={`block truncate text-sm font-semibold ${active ? 'text-white' : 'text-slate-800'}`}>{item.label}</span>
                      <span className={`mt-0.5 block truncate text-[10px] font-medium ${active ? 'text-white/75' : 'text-slate-400'}`}>{item.description}</span>
                    </span>
                  )}
        ...
        <div className="safe-area-bottom border-t border-slate-200/80 bg-slate-50/70 p-3 sm:p-4">
          {isOpen ? (
            <button
              ...
              className="flex min-h-11 w-full items-center gap-3 rounded-2xl border border-red-100 bg-white px-3 text-left text-red-600 shadow-sm transition hover:border-red-200 hover:bg-red-50 active:scale-[0.99] disabled:cursor-wait disabled:opacity-70"
            >
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-red-50">
                {isLoggingOut ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-red-300 border-t-red-600" /> : <LogOut className="h-4 w-4" />}
              </span>
              <span className="min-w-0 flex-1">
                <span className="block text-sm font-semibold">{isLoggingOut ? 'Disconnessione...' : 'Esci'}</span>
                <span className="block text-[10px] font-medium text-red-400">Disconnetti l'account</span>
              </span>
            </button>
          ) : (
            <button
              ...
              onClick={() => void handleLogout()}
              disabled={isLoggingOut}
              title="Disconnetti l'account"
              aria-label="Disconnetti l'account"
              className="mx-auto flex h-11 w-11 items-center justify-center rounded-xl border border-red-100 bg-white text-red-600 shadow-sm transition hover:border-red-200 hover:bg-red-50 active:scale-95 disabled:cursor-wait disabled:opacity-70"
            >
