...
export const AnnouncementServiceNotifier: React.FC<Props> = ({ onOpen }) => {
  ...
  const previousServiceRef = useRef<boolean | undefined>(user?.isOnService);

  const findUnreadLatest = useCallback(async () => {
    if (!user?.id || !user.isOnService) return null;
  ...
  useEffect(() => {
    ...
    const isOnService = user?.isOnService;
    if (isOnService && wasOnService === false) void notify();
    previousServiceRef.current = isOnService;
  }, [user?.isOnService, notify]);

  useEffect(() => {
    if (!user?.id) return;
    const channel = supabase.channel(`announcement-service-${user.id}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'announcements' }, () => {
        if (user.isOnService) void notify();
      })
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'users', filter: `id=eq.${user.id}` }, payload => {
        const oldValue = (payload.old as { is_on_service?: boolean }).is_on_service;
        const newValue = (payload.new as { is_on_service?: boolean }).is_on_service;
        if (newValue === true && oldValue === false) void notify();
      })
      .subscribe();
    return () => { void supabase.removeChannel(channel); };
  }, [user?.id, user?.isOnService, notify]);
