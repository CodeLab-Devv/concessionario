...
import { PresenceStatus, User, AuthContextType } from '../types';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
      ...
      try {
        ...
        if (!data) {
          ...
          if (!data) {
            ...
            if (insertRes.error) console.error('Direct upsert failed:', insertRes.error);
            data = insertRes.data;
          }
        }
  ...
  const login = useCallback(async (email: string, password: string): Promise<{ success: boolean; message?: string }> => {
    try {
      ...
      if (data.user) {
        ...
        if (!profileOk) return { success: false, message: 'Errore nel caricamento del profilo utente. Se il problema persiste, contatta l\'amministratore.' };
  ...
  const logout = useCallback(async () => {
    try {
      if (user?.isOnService) await toggleServiceStatus(user.id);
  ...
  const toggleServiceStatus = useCallback(async (userId?: string) => {
    ...
    try {
      ...
      await logActivity('Stato Servizio', `${userName} ${newStatus ? 'è entrato in servizio' : 'è uscito dal servizio'}`, targetUserId);
      return true;
    } catch (error) {
  ...
  const setPresenceStatus = useCallback(async (status: PresenceStatus): Promise<boolean> => {
    ...
    try {
      ...
      await logActivity('Stato Presenza', `${user.name} ha impostato lo stato su ${statusLabels[status]}`);
      return true;
    } catch (error) {
  ...
  const register = useCallback(async (email: string, password: string, name: string): Promise<{success: boolean, message?: string, needsEmailConfirmation?: boolean}> => {
    try {
      const { data: signUpData, error: authError } = await supabase.auth.signUp({ email, password, options: { data: { full_name: name } } });
      if (authError) {
      ...
      if (!authUser) return { success: false, message: 'Impossibile creare l\'account' };

      let profileCreated = true;
      try {
        const { error: rpcErr } = await supabase.rpc('create_user_profile', { p_id: authUser.id, p_email: email, p_name: name });
        if (rpcErr) {
  ...
  const resetAllData = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      await logActivity('Reset Completato', `${user.name} ha completato con successo il reset totale dei dati`);
      return true;
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="app-viewport bg-gray-50 flex items-center justify-center">
  ...
  return (
    <AuthContext.Provider value={{ user, login, logout, isAuthenticated, register, registerOwner, updateUserRole, fireEmployee, toggleServiceStatus, setPresenceStatus, fetchEmployeesStatus, resetAllData, employees, refreshUserProfile }}>
      {children}
