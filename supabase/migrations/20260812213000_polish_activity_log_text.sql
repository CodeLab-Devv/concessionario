...
BEGIN
  ...
  IF TG_TABLE_NAME = 'users' THEN
    ...
    THEN
      action_name := CASE
        WHEN COALESCE((new_payload ->> 'is_on_service')::BOOLEAN, FALSE) THEN 'Ingresso in servizio'
        ELSE 'Uscita dal servizio'
      END;
      detail_text := CASE
        WHEN COALESCE((new_payload ->> 'is_on_service')::BOOLEAN, FALSE)
          THEN format('%s è entrato in servizio.', COALESCE(target_name, actor_name, 'L''utente'))
        ELSE format('%s è uscito dal servizio.', COALESCE(target_name, actor_name, 'L''utente'))
      END;
    ELSIF TG_OP = 'UPDATE'
    ...
    ELSIF TG_OP = 'INSERT' THEN
      ...
      detail_text := format('Creato il profilo di %s.', COALESCE(target_name, 'utente'));
    ELSE
  ...
  ELSIF TG_TABLE_NAME = 'sales' THEN
    IF TG_OP = 'INSERT' THEN
      ...
      detail_text := format(
        'Registrata la vendita di %s%s: quantità %s, totale $ %s%s.',
    ...
    ELSIF TG_OP = 'UPDATE' THEN
      ...
      detail_text := format('Modificata la vendita di %s: quantità %s, totale $ %s.', COALESCE(payload ->> 'item_name', 'articolo'), COALESCE(payload ->> 'quantity', '1'), COALESCE(payload ->> 'total', '0'));
    ELSE
      action_name := 'Vendita eliminata';
      detail_text := format('Eliminata la vendita di %s: quantità %s, totale $ %s.', COALESCE(payload ->> 'item_name', 'articolo'), COALESCE(payload ->> 'quantity', '1'), COALESCE(payload ->> 'total', '0'));
    END IF;
  ...
  ELSIF TG_TABLE_NAME = 'announcements' THEN
    ...
    IF TG_OP = 'INSERT' THEN
      ...
      detail_text := format('Pubblicato l''annuncio «%s».', title_text);
    ELSIF TG_OP = 'UPDATE' THEN
  ...
  ELSIF TG_TABLE_NAME = 'daily_shifts' THEN
    ...
    detail_text := format(
      ...
      CASE TG_OP WHEN 'INSERT' THEN 'Assegnato il turno' WHEN 'UPDATE' THEN 'Modificato il turno' ELSE 'Rimosso il turno' END,
  ...
  ELSE
    action_name := CASE TG_OP WHEN 'INSERT' THEN 'Attività registrata' WHEN 'UPDATE' THEN 'Attività modificata' ELSE 'Attività rimossa' END;
    detail_text := 'È stata registrata una modifica nell''applicazione.';
  END IF;
  ...
  VALUES (
    ...
    TG_TABLE_NAME,
    record_key,
    jsonb_build_object('operation', TG_OP, 'old', old_payload, 'new', new_payload)
  );
...
-- Allinea anche i testi storici che mostravano una seconda ora nel dettaglio.
UPDATE public.activity_logs
SET details = regexp_replace(details, ' (alle|alle ore) [0-9]{2}/[0-9]{2}/[0-9]{4} [0-9]{2}:[0-9]{2}(:[0-9]{2})?\.', '.')
WHERE action IN ('Ingresso in servizio', 'Uscita dal servizio');
